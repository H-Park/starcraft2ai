...
def observation_to_gym_step(obs):
    gym_obs = obs["screen"][SCREEN_FEATURES.player_relative.index]
    gym_obs_extra_dims = np.expand_dims(gym_obs, axis=3)
    return gym_obs_extra_dims
...
class SC2AtariEnv:
    ...
    def state(self):
        timesteps = self.sc2_env.state
        return timestep_to_gym_step(timesteps[0])[0]

    def get_observation(self):
        obs = self.sc2_env._features.transform_obs(self.sc2_env._obs[0].observation)
        return observation_to_gym_step(obs)
