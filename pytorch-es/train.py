...
def do_rollouts(args, model, random_seeds, return_queue, env, are_negative):
    all_returns = []
    all_num_frames = []
    state = env.reset()
    this_model_return = 0
    ...
    for step in range(args.max_episode_length):
        ...
        logit = model((Variable(state.unsqueeze(0).float(), volatile=True)))
...
def gradient_update(args, synced_model, returns, random_seeds, neg_list,
    ...
    if args.useAdam:
        ...
        idx = 0
        for k, v in synced_model.es_params():
            r, _ = legacyOptim.adam( lambda x:  (1, -globalGrads[idx].cuda()), v , optimConfig[idx])
            v.copy_(r)
            idx = idx + 1
    else:
        # For each model, generate the same random numbers as we did
...
def render_env(model, env):
    while True:
        ...
        while not done:
            logit= model(Variable(state.unsqueeze(0).float(), volatile=True))
            prob = F.softmax(logit)
            value, action = prob.max(0)
            state, reward, done, _ = env.step(int(action.data[0]))
            this_model_return += reward
            state = torch.from_numpy(state)
...
def train_loop(args, synced_model, env, chkpt_dir):
    ...
    for _ in range(args.max_gradient_updates):
        ...
        for j in range(int(args.n/2)):
            random_seed, two_models = generate_seeds_and_models(args, synced_model, env[0])
        ...
        num_eps += len(results)
        synced_model = gradient_update(args, synced_model.cuda(), results, seeds,
